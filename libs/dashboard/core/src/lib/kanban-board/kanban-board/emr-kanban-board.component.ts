...
@Component({
  ...
  templateUrl: './emr-kanban-board.component.html',
  styleUrl: './emr-kanban-board.component.scss',
  host: {
    class: 'emr-kanban-board',
    '[class.is-dragging-active]': 'isDraggingActive',
  },
})
export class EmrKanbanBoardComponent<
  T extends KanbanColumn<K>,
  K extends KanbanItem,
> implements AfterViewInit
{
  private _elementRef = inject(ElementRef);
  protected _itemTplDef = contentChild.required(KanbanItemDefDirective);
  private _headerContainer = viewChild.required('headerContainer', {
    read: ElementRef,
  });
  private _scrollContainer = viewChild.required('scrollContainer', {
    read: ElementRef,
  });
  private _scrollContainerContent = viewChild.required(
  ...
  protected itemMousedown(event: MouseEvent) {
    const scrollContainerElement = this._scrollContainer()
    ...
    this._startContainerXOffset =
      targetRect.x - scrollContainerElement.getBoundingClientRect().x;
  ...
  onScroll(event: Event) {
    ...
    const headerContainerElement = this._headerContainer()
      .nativeElement as HTMLElement;
    this._renderer.setStyle(
      headerContainerElement,
      'transform',
      `translate(-${target.scrollLeft}px, 0)`,
    );
  ...
  onDragMoved(event: CdkDragMove<K>) {
    ...
    const itemOffsetXEnd =
      this._startContainerXOffset + event.distance.x + this._itemWidth;
