...
import { FilterBuilderOperationDefDirective, FilterBuilderOperationNameDirective } from '@core/directives';
import {
...
export class FilterBuilderComponent implements OnInit, AfterViewInit {
  ...
  private _resetMethodMap: {
    [prop: string]: (condition: FilterBuilderConditionInterface) => void;
  } = {
  ...
  readonly _prebuiltOperationDefs = viewChildren(
    FilterBuilderOperationDefDirective,
  );
  readonly _customOperationDefs = contentChildren(
    FilterBuilderOperationDefDirective,
  );
  ...
  ngOnInit() {
    if (this.value().length) {
      if (!this._isGroup(this.value()[0])) {
        throw new Error(
          'Invalid filter value, first element should be a filter group',
        );
      }
  ...
  ngAfterViewInit(): void {
    this._operationDefs = [
      ...this._prebuiltOperationDefs(),
      ...this._customOperationDefs(),
    ];
    this._operationDefs.forEach((operationDef) => {
      ...
      operationDef.allowedDataTypes().forEach((allowedType: string) => {
        ...
        const allowedTypeValue = this._operationAllowedTypesMap.get(
          allowedType,
        ) as string[];
  ...
  getConditionField(
    dataField: string,
  ): FilterBuilderFieldDefInterface | undefined {
  ...
  getSelectedGroupOperationName(
    targetGroup?: FilterBuilderGroupInterface,
  ): string {
    const groupLogicalOperatorId = targetGroup
      ? targetGroup.logicalOperator
      : this._logicalOperator;
    return (
      this.groupOperations().find(
        (groupOperator) => groupOperator.id === groupLogicalOperatorId,
      )?.name || ''
    );
  }

  selectConditionField(
    item: FilterBuilderConditionInterface,
    field: FilterBuilderFieldDefInterface,
  ): void {
    this.editItem = undefined;
    let allowedTypes = this._operationAllowedTypesMap.get(
      field.dataType,
    ) as string[];
  ...
  isOperationAllowedForCondition(
    dataField: string,
    operationId: string,
  ): boolean {
    const fieldDef = this.fieldDefs().find(
      (f) => f.dataField === dataField,
    ...
    if (!allowedTypes) {
      throw new Error(
        'There are not operations for the datatype: ' + fieldDef.dataType,
      );
    }
  ...
  getOptions(
    ...
    return fieldDef.lookup
      ?.dataSource as FilterBuilderFieldDataSourceItemInterface[];
  }

  getDataSourceItemNameById(
    item: FilterBuilderConditionInterface,
    ...
    return (
      (
        fieldDef.lookup
          ?.dataSource as FilterBuilderFieldDataSourceItemInterface[]
      ).find((item) => item.id === dataSourceItemId)?.name || ''
    );
  }
  ...
  protected operationIconTemplateRef(
    operation: FilterBuilderOperationDefDirective,
  ): TemplateRef<any> {
    return operation.operationIcon()?.templateRef as TemplateRef<any>;
  }

  protected operationNameTemplateRef(
    operation: FilterBuilderOperationDefDirective,
  ): TemplateRef<any> {
  ...
  protected _isGroup(
    item: FilterBuilderItemType,
  ): item is FilterBuilderGroupInterface {
    return 'logicalOperator' in item;
  }

  protected _isCondition(
    item: FilterBuilderItemType,
  ): item is FilterBuilderConditionInterface {
    return !('logicalOperator' in item);
  }
  ...
  private _getFieldDef(
    condition: FilterBuilderConditionInterface,
  ): FilterBuilderFieldDefInterface {
    return this.fieldDefs().find(
      (f) => f.dataField === condition['value'][0],
    ) as FilterBuilderFieldDefInterface;
  }
  ...
  private _resetValue(
    ...
    const resetMethod =
      '_reset' + this._capitalizeFirstLetter(fieldDef.dataType) + 'Value';
